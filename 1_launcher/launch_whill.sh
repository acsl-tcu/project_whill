...
if [[ ! "${TAG}" == image_* ]]; then
  ...
  $(echo "exec ros2 run whill_driver whill --log-opt max-size=100m --log-opt max-file=10 --ros-args --remap __node:=whill_node --remap __ns:=/$HOSTNAME --remap __whmodel:="CR"")
fi
