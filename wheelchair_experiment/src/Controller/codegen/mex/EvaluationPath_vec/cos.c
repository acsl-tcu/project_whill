/*
 * Academic License - for use in teaching, academic research, and meeting
 * course requirements at degree granting institutions only.  Not for
 * government, commercial, or other organizational use.
 *
 * cos.c
 *
 * Code generation for function 'cos'
 *
 */

/* Include files */
#include "cos.h"
#include "EvaluationPath_vec_data.h"
#include "EvaluationPath_vec_types.h"
#include "eml_int_forloop_overflow_check.h"
#include "rt_nonfinite.h"
#include "mwmathutil.h"

/* Variable Definitions */
static emlrtRSInfo hb_emlrtRSI = {
    11,                                                           /* lineNo */
    "cos",                                                        /* fcnName */
    "/usr/local/MATLAB/R2024b/toolbox/eml/lib/matlab/elfun/cos.m" /* pathName */
};

/* Function Definitions */
void b_cos(const emlrtStack *sp, emxArray_real_T *x)
{
  emlrtStack b_st;
  emlrtStack c_st;
  emlrtStack st;
  real_T *x_data;
  int32_T k;
  int32_T nx_tmp;
  st.prev = sp;
  st.tls = sp->tls;
  b_st.prev = &st;
  b_st.tls = st.tls;
  c_st.prev = &b_st;
  c_st.tls = b_st.tls;
  x_data = x->data;
  st.site = &hb_emlrtRSI;
  nx_tmp = x->size[0] * x->size[1];
  b_st.site = &fb_emlrtRSI;
  if (nx_tmp > 2147483646) {
    c_st.site = &gb_emlrtRSI;
    check_forloop_overflow_error(&c_st);
  }
  for (k = 0; k < nx_tmp; k++) {
    x_data[k] = muDoubleScalarCos(x_data[k]);
  }
}

void c_cos(const emlrtStack *sp, emxArray_real_T *x)
{
  emlrtStack b_st;
  emlrtStack c_st;
  emlrtStack st;
  real_T *x_data;
  int32_T k;
  int32_T nx_tmp;
  st.prev = sp;
  st.tls = sp->tls;
  b_st.prev = &st;
  b_st.tls = st.tls;
  c_st.prev = &b_st;
  c_st.tls = b_st.tls;
  x_data = x->data;
  st.site = &hb_emlrtRSI;
  nx_tmp = x->size[2] * x->size[3];
  b_st.site = &fb_emlrtRSI;
  if (nx_tmp > 2147483646) {
    c_st.site = &gb_emlrtRSI;
    check_forloop_overflow_error(&c_st);
  }
  for (k = 0; k < nx_tmp; k++) {
    x_data[k] = muDoubleScalarCos(x_data[k]);
  }
}

void d_cos(real_T x_data[], const int32_T x_size[3])
{
  int32_T i;
  int32_T k;
  i = x_size[0] * x_size[2];
  for (k = 0; k < i; k++) {
    x_data[k] = muDoubleScalarCos(x_data[k]);
  }
}

void e_cos(const emlrtStack *sp, emxArray_real_T *x)
{
  emlrtStack b_st;
  emlrtStack c_st;
  emlrtStack st;
  real_T *x_data;
  int32_T k;
  int32_T nx_tmp;
  st.prev = sp;
  st.tls = sp->tls;
  b_st.prev = &st;
  b_st.tls = st.tls;
  c_st.prev = &b_st;
  c_st.tls = b_st.tls;
  x_data = x->data;
  st.site = &hb_emlrtRSI;
  nx_tmp = x->size[0] * x->size[2] * x->size[3];
  b_st.site = &fb_emlrtRSI;
  if (nx_tmp > 2147483646) {
    c_st.site = &gb_emlrtRSI;
    check_forloop_overflow_error(&c_st);
  }
  for (k = 0; k < nx_tmp; k++) {
    x_data[k] = muDoubleScalarCos(x_data[k]);
  }
}

/* End of code generation (cos.c) */
