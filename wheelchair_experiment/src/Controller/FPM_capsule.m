function [Grade,Grade_old] = FPM_capsule(obj,x,y,th,obs_now,target,param_FPM)
% function [Grade,Grade_old,phio_min,phio_max,grade_trgt,grade_obs,d_c1c2] = FPM_capsule(a_wheel,b_wheel,a_obs,b_obs,obsnum,x,y,th,OBS_NOW,tho,goal)  % テスト用
    ...
    margin = param_FPM.margin;      % PMF for the Obstacle の障害物のマージン
    ...
    OBS_NOW     = obs_now(:,1:2);               % 歩行者の位置
    a_obs       = obs_now(:,6);                 % 歩行者の横幅
    b_obs       = obs_now(:,7)+margin;          % 歩行者の縦幅
...
%--PFM of Obstacle (begin)-------------------------------------------------
    ...
    %     d_o(i,1) = norm(OBS_NOW_2(i,:));                                             
    % end
    OBS_NOW_2 = (R*[OBS_NOW(:,1)-state(1),OBS_NOW(:,2)-state(2)]')';    % 障害物の位置 (車両位置が原点，車両の方向角がx軸正)
    d_o = vecnorm(OBS_NOW_2,2,2);                                       % 車両と障害物の距離
    ...
    d_c1 = vecnorm(c1_3,2,2);                       % 車両と円1との距離
    d_c2 = vecnorm(c2_3,2,2);                       % 車両と円2との距離
    ...
    for i = 1:obsnum
        if d_c1c2(i,1) >= a_wheel+b_obs(i)   % 通常(車両と障害物が重なっていない場合)
            phi_c1_max = phi_c1(i,1) + asin((b_obs(i) + a_wheel) / d_c1(i,1));    % [車両と障害物を結ぶ線] と [車両と円1の接線1] のなす角
            phi_c1_min = phi_c1(i,1) - asin((b_obs(i) + a_wheel) / d_c1(i,1));    % [車両と障害物を結ぶ線] と [車両と円1の接線2] のなす角
            phi_c2_max = phi_c2(i,1) + asin((b_obs(i) + a_wheel) / d_c2(i,1));    % [車両と障害物を結ぶ線] と [車両と円2の接線1] のなす角
            phi_c2_min = phi_c2(i,1) - asin((b_obs(i) + a_wheel) / d_c2(i,1));    % [車両と障害物を結ぶ線] と [車両と円2の接線2] のなす角
            ...
            phio_max(i,1) = phio_2(i,1) + (pi-asin(d_c1c2(i,1)/(a_wheel+b_obs(i))));
            phio_min(i,1) = phio_2(i,1) - (pi-asin(d_c1c2(i,1)/(a_wheel+b_obs(i))));
        end
    end
    

    % for i = 1:obsnum
    %     if d_c1c2(i,1) < a_wheel+b_obs
    %         if (phio_min(i,1) <= 0) && (0 <= phio_max(i,1))
    %             phio_max(i,1) = phio_2(i,1) + (pi-asin(d_c1c2(i,1)/(a_wheel+b_obs)))
    %             phio_min(i,1) = phio_2(i,1) - (pi-asin(d_c1c2(i,1)/(a_wheel+b_obs)))
    %             disp("pass");
    %         else
    %             disp("No");
    %             i
    ...
    for i = 1:obsnum
        if (phio_min(i,1) <= 0) && (0 <= phio_max(i,1))
            if d_o(i,1) < alpha     % alpha よりも障害物が遠いい場合
                % grade_obs(i,1) = 1 - 100*prob(i);   % PMF of Obstacle
                grade_obs(i,1) =  1-exp(-(d_c1c2(i,1)-b_obs(i)-r_wheel)/1);    % テスト用
                % grade_obs(i,1) = (alpha-d_c1c2(i,1))/(b_obs(i)+r_wheel);
                % grade_obs(i,1) = 0;    % テスト用

                % grade_obs_center = 1-exp(-(d_c1c2(i,1)-b_obs(i,1)-0.76)/4);
                % d_th = abs(phio_2(i));
                % grade_obs(i,1) = grade_obs_center+d_th*tan(1/20*pi);
                if d_c1c2(i,1)-b_obs(i,1)-r_wheel <=0
                    grade_obs(i,1) = grade_obs(i,1)-obs_cost;
                end
            else                    % alpha よりも障害物が遠いい場合
                grade_obs(i,1) = 1.0;
            end
...
%--------------------------------------------------------------------------
    ...
    function [distance, closest_point] = distance_to_origin(x1, y1, x2, y2)
        % 原点から線分までの距離と、線分上で原点に最も近い点を求める関数
        %
        % 入力:
        % (x1, y1) - 線分の始点の座標
        % (x2, y2) - 線分の終点の座標
        %
        % 出力:
        % distance - 原点から線分までの最小距離
        % closest_point - 線分上で原点に最も近い点の座標
