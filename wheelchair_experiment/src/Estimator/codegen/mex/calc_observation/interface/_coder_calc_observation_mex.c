 ...
 *
 * _coder_calc_observation_mex.c
 *
 * Code generation for function '_coder_calc_observation_mex'
 *
 */
...
void calc_observation_mexFunction(int32_T nlhs, mxArray *plhs[2], int32_T nrhs,
                                  const mxArray *prhs[11])
{
  ...
  /* Call the function. */
  calc_observation_api(prhs, nlhs, outputs);
                 ...
                 const mxArray *prhs[])
{
  ...
  mexAtExit(&calc_observation_atexit);
  emlrtLoadMATLABLibrary("sys/os/glnxa64/libiomp5.so");
  /* Initialize the memory manager. */
  omp_init_lock(&emlrtLockGlobal);
  omp_init_nest_lock(&calc_observation_nestLockGlobal);
  calc_observation_initialize();
  st.tls = emlrtRootTLSGlobal;
  emlrtSetJmpBuf(&st, &emlrtJBEnviron);
  if (setjmp(emlrtJBEnviron) == 0) {
    calc_observation_mexFunction(nlhs, plhs, nrhs, prhs);
    calc_observation_terminate();
...
/* End of code generation (_coder_calc_observation_mex.c) */
