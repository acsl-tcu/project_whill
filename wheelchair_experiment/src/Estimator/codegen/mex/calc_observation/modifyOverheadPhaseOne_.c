...
void modifyOverheadPhaseOne_(const emlrtStack *sp, g_struct_T *obj)
{
  ...
  if ((obj->sizes[3] < 1) || (obj->sizes[3] > 8)) {
    emlrtDynamicBoundsCheckR2012b(obj->sizes[3], 1, 8, &vb_emlrtBCI,
                                  (emlrtConstCTX)sp);
  }
  ...
  for (idx = idxStartIneq; idx <= b; idx++) {
    i = obj->nVar + ((idx - 1) << 3);
    if ((i < 1) || (i > 120)) {
      emlrtDynamicBoundsCheckR2012b(i, 1, 120, &ub_emlrtBCI, (emlrtConstCTX)sp);
    }
    obj->ATwset[i - 1] = -1.0;
  }
  ...
  if (obj->nWConstr[4] > 0) {
    ...
    for (idx = i; idx >= 1; idx--) {
      b = idxStartIneq + idx;
      if ((b - 1 < 1) || (b - 1 > 15)) {
        emlrtDynamicBoundsCheckR2012b(b - 1, 1, 15, &xb_emlrtBCI,
                                      (emlrtConstCTX)sp);
      }
      if ((b < 1) || (b > 15)) {
        emlrtDynamicBoundsCheckR2012b(b, 1, 15, &wb_emlrtBCI,
                                      (emlrtConstCTX)sp);
      }
    ...
    if ((i < 1) || (i > 15)) {
      emlrtDynamicBoundsCheckR2012b(i, 1, 15, &wb_emlrtBCI, (emlrtConstCTX)sp);
    }
    obj->isActiveConstr[i - 1] = false;
  }
  if ((obj->isActiveIdx[4] < 1) || (obj->isActiveIdx[4] > 15)) {
    emlrtDynamicBoundsCheckR2012b(obj->isActiveIdx[4], 1, 15, &wb_emlrtBCI,
                                  (emlrtConstCTX)sp);
  }
