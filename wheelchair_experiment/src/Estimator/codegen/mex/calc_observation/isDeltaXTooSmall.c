...
boolean_T isDeltaXTooSmall(const emlrtStack *sp, const real_T xCurrent[5],
                           const real_T delta_x[8], int32_T nVar)
{
  emlrtStack b_st;
  emlrtStack st;
  int32_T idx;
  ...
  while ((!exitg1) && (idx <= (uint8_T)nVar - 1)) {
    if (idx + 1 > 5) {
      emlrtDynamicBoundsCheckR2012b(idx + 1, 1, 5, &cb_emlrtBCI,
                                    (emlrtConstCTX)sp);
    }
