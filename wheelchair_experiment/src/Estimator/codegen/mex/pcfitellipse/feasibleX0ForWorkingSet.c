...
boolean_T feasibleX0ForWorkingSet(const emlrtStack *sp, real_T workspace[120],
                                  real_T xCurrent[8], k_struct_T *workingset,
                                  g_struct_T *qrmanager)
{
  ...
  if (mWConstr != 0) {
    ...
    for (offsetQR = 0; offsetQR < mWConstr; offsetQR++) {
      ...
      if ((offsetQR + 1 < 1) || (offsetQR + 1 > 15)) {
        emlrtDynamicBoundsCheckR2012b(offsetQR + 1, 1, 15, &mc_emlrtBCI,
                                      (emlrtConstCTX)sp);
      }
      workspace[offsetQR] = c;
      workspace[offsetQR + 15] = c;
    }
    st.site = &hf_emlrtRSI;
    b_st.site = &sc_emlrtRSI;
    if (mWConstr != 0) {
      b_tmp = ((mWConstr - 1) << 3) + 1;
    ...
    if (mWConstr >= nVar) {
      ...
      for (iac = 0; iac < k; iac++) {
        ...
        for (idx_row = 0; idx_row < mWConstr; idx_row++) {
          b_tmp = (iac + (idx_row << 3)) + 1;
          if ((b_tmp < 1) || (b_tmp > 120)) {
            emlrtDynamicBoundsCheckR2012b(b_tmp, 1, 120, &nc_emlrtBCI,
                                          (emlrtConstCTX)sp);
          }
          ia = idx_row + offsetQR;
          if ((ia < 1) || (ia > 225)) {
            emlrtDynamicBoundsCheckR2012b(ia, 1, 225, &oc_emlrtBCI,
                                          (emlrtConstCTX)sp);
          }
      ...
      c = 1.0;
      memcpy(&B[0], &workspace[0], 120U * sizeof(real_T));
      beta1 = 0.0;
      DIAGA1 = 'N';
      TRANSA1 = 'T';
      m_t = (ptrdiff_t)nVar;
    ...
    do {
      ...
      if (offsetQR <= (uint8_T)nVar - 1) {
        ...
        if ((c <= 2.2204460492503131E-16) || (c < beta1)) {
          ...
          c_st.site = &xc_emlrtRSI;
          k = (uint8_T)nVar;
          memcpy(&xCurrent[0], &workspace[15], (uint32_T)k * sizeof(real_T));
        }
        exitg1 = 1;
      }
    } while (exitg1 == 0);
