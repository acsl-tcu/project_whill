...
void computeGrad_StoreHx(const emlrtStack *sp, b_struct_T *obj,
                         ...
                         const real_T x[8])
{
  ...
  case 5:
    ...
    if ((obj->nvar < 1) || (obj->nvar > 8)) {
      emlrtDynamicBoundsCheckR2012b(obj->nvar, 1, 8, &yc_emlrtBCI,
                                    (emlrtConstCTX)sp);
    }
  ...
  case 3: {
    ...
    for (ixlast = 0; ixlast < i; ixlast++) {
      if (ixlast + 1 > 7) {
        emlrtDynamicBoundsCheckR2012b(ixlast + 1, 1, 7, &ad_emlrtBCI,
                                      (emlrtConstCTX)sp);
      }
