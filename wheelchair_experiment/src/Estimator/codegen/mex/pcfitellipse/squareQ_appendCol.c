...
void squareQ_appendCol(const emlrtStack *sp, g_struct_T *obj,
                       const real_T vec[120], int32_T iv0)
{
  ...
  if ((obj->ncols < 1) || (obj->ncols > 15)) {
    emlrtDynamicBoundsCheckR2012b(obj->ncols, 1, 15, &dd_emlrtBCI,
                                  (emlrtConstCTX)sp);
  }
  ia = obj->ncols - 1;
  ...
  for (idx = obj->mrows; idx > obj->ncols; idx--) {
    ...
    if ((iyend - 1 < 1) || (iyend - 1 > 225)) {
      emlrtDynamicBoundsCheckR2012b(iyend - 1, 1, 225, &ed_emlrtBCI, &st);
    }
    temp_tmp = obj->QR[iyend - 2];
    if ((iyend < 1) || (iyend > 225)) {
      emlrtDynamicBoundsCheckR2012b(iyend, 1, 225, &ed_emlrtBCI, &st);
    }
    temp = obj->QR[iyend - 1];
