...
void fullColLDL2_(const emlrtStack *sp, f_struct_T *obj, int32_T NColsRemain)
{
  ...
  for (k = 0; k < NColsRemain; k++) {
    ...
    int32_T vectorUB;
    LD_diagOffset = 14 * k;
    if ((LD_diagOffset + 1 < 1) || (LD_diagOffset + 1 > 169)) {
      emlrtDynamicBoundsCheckR2012b(LD_diagOffset + 1, 1, 169, &ye_emlrtBCI,
                                    (emlrtConstCTX)sp);
    }
  ...
  jA = 14 * (NColsRemain - 1) + 1;
  if ((jA < 1) || (jA > 169)) {
    emlrtDynamicBoundsCheckR2012b(jA, 1, 169, &ye_emlrtBCI, (emlrtConstCTX)sp);
  }
}
