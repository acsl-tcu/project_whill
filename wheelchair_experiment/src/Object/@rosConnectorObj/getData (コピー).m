...
try
    switch obj.Mode
        case {'Physics',   2}
            ...
            %%%% Autoware only %%%%
            if obj.Autoware(1) == true && obj.Sensor(obj.Prime) == false
                ...
                ret.yaw   = double(eulZYX(1)) + 0.0111;                                                                                                                                                                                                                                                                                                                                                      
                if obj.Count > 2
                ...
                %%%% True and Autoware %%%%
            elseif obj.Autoware(1) == 2 && obj.Sensor(obj.Prime) == true
                gtp{obj.ndtpose}   = obj.subTopic{obj.ndtpose}.LatestMessage;
                ...
                ret.AutowareX = double(gtp{obj.ndtpose}.Pose.Position.X) + L * cos(ret.Autowareyaw - theta) + 3.50;
                ret.AutowareY = double(gtp{obj.ndtpose}.Pose.Position.Y) + L * sin(ret.Autowareyaw - theta) - 0.08;
                ...
                %%%% Autoware and True %%%%
            elseif obj.Autoware(1) == true && obj.Sensor(obj.Prime) == true
                gtp{obj.pose}    = obj.subTopic{obj.pose}.LatestMessage;
                gtp{obj.ndtpose}   = obj.subTopic{obj.ndtpose}.LatestMessage;
                ...
                x = ret.X;
                y = ret.Y;
                [x_global, y_global] = pcmaptogrobal(yerr, x, y);
...
%     				ret.gyr(3) = gtp{obj.gyr}.Z;
        ...
        case {'Expriment', 3}
            ...
            if obj.Autoware(1) == true && obj.Sensor(obj.Prime) == false
                ...
                ret.roll  = double(eulZYX(3));
                ret.pitch = double(eulZYX(2));
                ret.yaw   = double(eulZYX(1)) + 1.442337e-02;
                
                if obj.Count > 1
                     ret = AngleAdjstment(ret, obj);
                end

                ret.X = double(gtp{obj.ndtpose}.Pose.Position.X) + L * cos(ret.yaw - theta) + 4.542500e-02;
                ret.Y = double(gtp{obj.ndtpose}.Pose.Position.Y) + L * sin(ret.yaw - theta) + 4.811500e-02;
                ret.Z = double(gtp{obj.ndtpose}.Pose.Position.Z);
...
%     				ret.gyr(3) = gtp{obj.gyr}.Z;
    ...
    if obj.Sensor(obj.RealSence) == true
     ...
     if obj.Autoware(2) == true
        ret.next_target= obj.subTopic{obj.next_target}.LatestMessage.Pose.Position;
    end
catch ME
    endExperiment(obj);
