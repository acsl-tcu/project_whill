...
static emlrtRSInfo udb_emlrtRSI = { 98,// lineNo
  "cvmeasjac",                         // fcnName
  "/usr/local/MATLAB/R2020b/toolbox/shared/tracking/fusionlib/cvmeasjac.m"// pathName 
};
...
namespace coder
{
  void cvmeasjac(const emlrtStack *sp, const real_T state[6], real_T jacobian[18])
  {
    ...
    for (int32_T i = 0; i < 3; i++) {
      k = 3 * (((i + 1) << 1) - 2);
      jacobian[k] = iv[3 * i];
      jacobian[k + 1] = iv[3 * i + 1];
      jacobian[k + 2] = iv[3 * i + 2];
    }
  }
}

// End of code generation (cvmeasjac.cpp)
