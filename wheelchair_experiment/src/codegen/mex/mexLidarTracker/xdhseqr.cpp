...
static emlrtRSInfo qp_emlrtRSI = { 365,// lineNo
  "eml_dlahqr",                        // fcnName
      ...
      int32_T eml_dlahqr(const emlrtStack *sp, real_T h[36])
      {
        ...
        while ((!exitg1) && (i + 1 >= 1)) {
          ...
          while ((!exitg2) && (its < 301)) {
            ...
            while ((!exitg3) && (k + 1 > L)) {
              sum1_tmp = k + 6 * (k - 1);
              ba = muDoubleScalarAbs(h[sum1_tmp]);
              if (ba <= 6.0125050800269183E-292) {
                exitg3 = true;
              } else {
                hoffset = k + 6 * k;
                bb = muDoubleScalarAbs(h[hoffset]);
                tst = muDoubleScalarAbs(h[sum1_tmp - 1]) + bb;
                if (tst == 0.0) {
                  if (k - 1 >= 1) {
                ...
                if (ba <= 2.2204460492503131E-16 * tst) {
                  ...
                  tst = muDoubleScalarAbs(h[sum1_tmp - 1] - h[hoffset]);
                  if (bb > tst) {
                    aa = bb;
                    bb = tst;
                  } else {
            ...
            if (k + 1 >= i) {
              ...
              if (its == 10) {
                hoffset = k + 6 * k;
                s = muDoubleScalarAbs(h[hoffset + 1]) + muDoubleScalarAbs(h[(k +
                  6 * (k + 1)) + 2]);
                ...
                s = muDoubleScalarAbs(h[i + 6 * (i - 1)]) + muDoubleScalarAbs(h
                  [(i + 6 * (i - 2)) - 1]);
                tst = 0.75 * s + h[i + 6 * i];
                aa = -0.4375 * s;
                ab = s;
              ...
              if (s == 0.0) {
                ...
                if (tst >= 0.0) {
                  ...
                  rt1r = ba + ab;
                  rt2r = ba - ab;
                  if (muDoubleScalarAbs(rt1r - bb) <= muDoubleScalarAbs(rt2r -
                       bb)) {
                    rt1r *= s;
                    rt2r = rt1r;
                  } else {
              ...
              while ((!exitg3) && (m >= k + 1)) {
                ...
                if (m == k + 1) {
                  ...
                  sum1_tmp = m + 6 * (m - 2);
                  if (muDoubleScalarAbs(h[sum1_tmp - 1]) * (muDoubleScalarAbs(v
                        [1]) + muDoubleScalarAbs(v[2])) <=
                      2.2204460492503131E-16 * muDoubleScalarAbs(v[0]) *
                      ((muDoubleScalarAbs(h[sum1_tmp - 2]) + muDoubleScalarAbs
                        (s_tmp)) + muDoubleScalarAbs(tst))) {
                    exitg3 = true;
                  } else {
                    m--;
                  }
              ...
              for (int32_T b_k = m; b_k <= i; b_k++) {
                ...
                if (nr == 3) {
                  ...
                  for (j = b_k; j < 7; j++) {
                    hoffset = b_k + 6 * (j - 1);
                    aa = (h[hoffset - 1] + rt1r * h[hoffset]) + s * h[hoffset +
                      1];
                    h[hoffset - 1] -= aa * bb;
                    h[hoffset] -= aa * tst;
                    h[hoffset + 1] -= aa * ba;
                  ...
                  hoffset = b_k + 3;
                  sum1_tmp = i + 1;
                  nr = muIntScalarMin_sint32(hoffset, sum1_tmp);
                  for (j = 0; j < nr; j++) {
                    int32_T b_sum1_tmp;
                    hoffset = j + 6 * (b_k - 1);
                    ab = h[hoffset];
                    sum1_tmp = j + 6 * b_k;
                    b_sum1_tmp = j + 6 * (b_k + 1);
                    aa = (ab + rt1r * h[sum1_tmp]) + s * h[b_sum1_tmp];
                    h[hoffset] = ab - aa * bb;
                    h[sum1_tmp] -= aa * tst;
                    h[b_sum1_tmp] -= aa * ba;
                  }
          ...
          if (!goto150) {
            ...
            if ((L != i + 1) && (L == i)) {
              ...
              s = h[hoffset];
              tst = h[sum1_tmp];
              xdlanv2(&h[(i + 6 * (i - 1)) - 1], &rt1r, &s, &tst, &ab, &aa, &ba,
                      &bb, &s_tmp, &rt2r);
              h[sum1_tmp - 1] = rt1r;
              h[hoffset] = s;
              h[sum1_tmp] = tst;
              if (6 > i + 1) {
                st.site = &rp_emlrtRSI;
                blas::b_xrot(&st, 5 - i, h, i + (i + 1) * 6, (i + (i + 1) * 6) +
                             1, s_tmp, rt2r);
              }

              st.site = &qp_emlrtRSI;
              blas::c_xrot(&st, i - 1, h, (i - 1) * 6 + 1, i * 6 + 1, s_tmp,
                           rt2r);
            }
