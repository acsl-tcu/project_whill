/*
 * Academic License - for use in teaching, academic research, and meeting
 * course requirements at degree granting institutions only.  Not for
 * government, commercial, or other organizational use.
 *
 * numPotentialFeasibleEvents.c
 *
 * Code generation for function 'numPotentialFeasibleEvents'
 *
 */

/* Include files */
#include "numPotentialFeasibleEvents.h"
#include "eml_int_forloop_overflow_check.h"
#include "mexLidarTracker_data.h"
#include "mexLidarTracker_emxutil.h"
#include "mexLidarTracker_types.h"
#include "nchoosek.h"
#include "rt_nonfinite.h"
#include "mwmathutil.h"

/* Variable Definitions */
static emlrtRSInfo dw_emlrtRSI = {
    15,                           /* lineNo */
    "numPotentialFeasibleEvents", /* fcnName */
    "/usr/local/MATLAB/R2023a/toolbox/fusion/fusion/+fusion/+internal/"
    "+assignment/numPotentialFeasibleEvents.m" /* pathName */
};

static emlrtRSInfo ew_emlrtRSI = {
    18,                           /* lineNo */
    "numPotentialFeasibleEvents", /* fcnName */
    "/usr/local/MATLAB/R2023a/toolbox/fusion/fusion/+fusion/+internal/"
    "+assignment/numPotentialFeasibleEvents.m" /* pathName */
};

static emlrtRSInfo kw_emlrtRSI = {
    10,          /* lineNo */
    "factorial", /* fcnName */
    "/usr/local/MATLAB/R2023a/toolbox/eml/lib/matlab/specfun/factorial.m" /* pathName
                                                                           */
};

static emlrtRSInfo lw_emlrtRSI = {
    17,                           /* lineNo */
    "applyScalarFunctionInPlace", /* fcnName */
    "/usr/local/MATLAB/R2023a/toolbox/eml/eml/+coder/+internal/"
    "applyScalarFunctionInPlace.m" /* pathName */
};

static emlrtRSInfo rw_emlrtRSI = {
    11,     /* lineNo */
    "prod", /* fcnName */
    "/usr/local/MATLAB/R2023a/toolbox/eml/lib/matlab/datafun/prod.m" /* pathName
                                                                      */
};

static emlrtRTEInfo bc_emlrtRTEI = {
    21,                 /* lineNo */
    5,                  /* colNo */
    "scalar_factorial", /* fName */
    "/usr/local/MATLAB/R2023a/toolbox/eml/lib/matlab/specfun/factorial.m" /* pName
                                                                           */
};

static emlrtRTEInfo qh_emlrtRTEI = {
    18,                           /* lineNo */
    15,                           /* colNo */
    "numPotentialFeasibleEvents", /* fName */
    "/usr/local/MATLAB/R2023a/toolbox/fusion/fusion/+fusion/+internal/"
    "+assignment/numPotentialFeasibleEvents.m" /* pName */
};

static emlrtRTEInfo sh_emlrtRTEI = {
    18,                           /* lineNo */
    11,                           /* colNo */
    "numPotentialFeasibleEvents", /* fName */
    "/usr/local/MATLAB/R2023a/toolbox/fusion/fusion/+fusion/+internal/"
    "+assignment/numPotentialFeasibleEvents.m" /* pName */
};

/* Function Definitions */
int32_T numPotentialFeasibleEvents(const emlrtStack *sp,
                                   const emxArray_boolean_T *validationMatrix,
                                   int32_T numMeas, int32_T numTracks)
{
  static const real_T dv[170] = {1.0,
                                 2.0,
                                 6.0,
                                 24.0,
                                 120.0,
                                 720.0,
                                 5040.0,
                                 40320.0,
                                 362880.0,
                                 3.6288E+6,
                                 3.99168E+7,
                                 4.790016E+8,
                                 6.2270208E+9,
                                 8.71782912E+10,
                                 1.307674368E+12,
                                 2.0922789888E+13,
                                 3.55687428096E+14,
                                 6.402373705728E+15,
                                 1.21645100408832E+17,
                                 2.43290200817664E+18,
                                 5.109094217170944E+19,
                                 1.1240007277776077E+21,
                                 2.5852016738884978E+22,
                                 6.2044840173323941E+23,
                                 1.5511210043330986E+25,
                                 4.0329146112660565E+26,
                                 1.0888869450418352E+28,
                                 3.0488834461171384E+29,
                                 8.8417619937397008E+30,
                                 2.6525285981219103E+32,
                                 8.2228386541779224E+33,
                                 2.6313083693369352E+35,
                                 8.6833176188118859E+36,
                                 2.9523279903960412E+38,
                                 1.0333147966386144E+40,
                                 3.7199332678990118E+41,
                                 1.3763753091226343E+43,
                                 5.23022617466601E+44,
                                 2.0397882081197442E+46,
                                 8.1591528324789768E+47,
                                 3.3452526613163803E+49,
                                 1.4050061177528798E+51,
                                 6.0415263063373834E+52,
                                 2.6582715747884485E+54,
                                 1.1962222086548019E+56,
                                 5.5026221598120885E+57,
                                 2.5862324151116818E+59,
                                 1.2413915592536073E+61,
                                 6.0828186403426752E+62,
                                 3.0414093201713376E+64,
                                 1.5511187532873822E+66,
                                 8.0658175170943877E+67,
                                 4.2748832840600255E+69,
                                 2.3084369733924138E+71,
                                 1.2696403353658276E+73,
                                 7.1099858780486348E+74,
                                 4.0526919504877221E+76,
                                 2.3505613312828789E+78,
                                 1.3868311854568986E+80,
                                 8.3209871127413916E+81,
                                 5.0758021387722484E+83,
                                 3.1469973260387939E+85,
                                 1.98260831540444E+87,
                                 1.2688693218588417E+89,
                                 8.2476505920824715E+90,
                                 5.4434493907744307E+92,
                                 3.6471110918188683E+94,
                                 2.4800355424368305E+96,
                                 1.711224524281413E+98,
                                 1.197857166996989E+100,
                                 8.5047858856786218E+101,
                                 6.1234458376886077E+103,
                                 4.4701154615126834E+105,
                                 3.3078854415193856E+107,
                                 2.4809140811395391E+109,
                                 1.8854947016660498E+111,
                                 1.4518309202828584E+113,
                                 1.1324281178206295E+115,
                                 8.9461821307829729E+116,
                                 7.1569457046263779E+118,
                                 5.7971260207473655E+120,
                                 4.75364333701284E+122,
                                 3.9455239697206569E+124,
                                 3.314240134565352E+126,
                                 2.8171041143805494E+128,
                                 2.4227095383672724E+130,
                                 2.1077572983795269E+132,
                                 1.8548264225739836E+134,
                                 1.6507955160908452E+136,
                                 1.4857159644817607E+138,
                                 1.3520015276784023E+140,
                                 1.24384140546413E+142,
                                 1.1567725070816409E+144,
                                 1.0873661566567424E+146,
                                 1.0329978488239052E+148,
                                 9.916779348709491E+149,
                                 9.6192759682482062E+151,
                                 9.426890448883242E+153,
                                 9.33262154439441E+155,
                                 9.33262154439441E+157,
                                 9.4259477598383536E+159,
                                 9.6144667150351211E+161,
                                 9.9029007164861754E+163,
                                 1.0299016745145622E+166,
                                 1.0813967582402903E+168,
                                 1.1462805637347078E+170,
                                 1.2265202031961373E+172,
                                 1.3246418194518284E+174,
                                 1.4438595832024928E+176,
                                 1.5882455415227421E+178,
                                 1.7629525510902437E+180,
                                 1.9745068572210728E+182,
                                 2.2311927486598123E+184,
                                 2.5435597334721862E+186,
                                 2.9250936934930141E+188,
                                 3.3931086844518965E+190,
                                 3.969937160808719E+192,
                                 4.6845258497542883E+194,
                                 5.5745857612076033E+196,
                                 6.6895029134491239E+198,
                                 8.09429852527344E+200,
                                 9.8750442008335976E+202,
                                 1.2146304367025325E+205,
                                 1.5061417415111404E+207,
                                 1.8826771768889254E+209,
                                 2.3721732428800459E+211,
                                 3.0126600184576582E+213,
                                 3.8562048236258025E+215,
                                 4.9745042224772855E+217,
                                 6.4668554892204716E+219,
                                 8.4715806908788174E+221,
                                 1.1182486511960039E+224,
                                 1.4872707060906852E+226,
                                 1.9929427461615181E+228,
                                 2.6904727073180495E+230,
                                 3.6590428819525472E+232,
                                 5.01288874827499E+234,
                                 6.9177864726194859E+236,
                                 9.6157231969410859E+238,
                                 1.346201247571752E+241,
                                 1.89814375907617E+243,
                                 2.6953641378881614E+245,
                                 3.8543707171800706E+247,
                                 5.5502938327393013E+249,
                                 8.0479260574719866E+251,
                                 1.17499720439091E+254,
                                 1.7272458904546376E+256,
                                 2.5563239178728637E+258,
                                 3.8089226376305671E+260,
                                 5.7133839564458505E+262,
                                 8.6272097742332346E+264,
                                 1.3113358856834518E+267,
                                 2.0063439050956811E+269,
                                 3.0897696138473489E+271,
                                 4.7891429014633912E+273,
                                 7.47106292628289E+275,
                                 1.1729568794264138E+278,
                                 1.8532718694937338E+280,
                                 2.9467022724950369E+282,
                                 4.714723635992059E+284,
                                 7.5907050539472148E+286,
                                 1.2296942187394488E+289,
                                 2.0044015765453015E+291,
                                 3.2872185855342945E+293,
                                 5.423910666131586E+295,
                                 9.0036917057784329E+297,
                                 1.5036165148649983E+300,
                                 2.5260757449731969E+302,
                                 4.2690680090047027E+304,
                                 7.257415615307994E+306};
  emlrtStack b_st;
  emlrtStack c_st;
  emlrtStack d_st;
  emlrtStack e_st;
  emlrtStack f_st;
  emlrtStack g_st;
  emlrtStack st;
  emxArray_int8_T *x;
  emxArray_real_T *y;
  real_T n;
  real_T n1;
  real_T n2;
  real_T *y_data;
  int32_T k;
  int32_T maxval;
  int32_T nPotentials;
  int32_T vstride;
  int32_T xj;
  int32_T xoffset;
  int8_T *x_data;
  const boolean_T *validationMatrix_data;
  st.prev = sp;
  st.tls = sp->tls;
  b_st.prev = &st;
  b_st.tls = st.tls;
  c_st.prev = &b_st;
  c_st.tls = b_st.tls;
  d_st.prev = &c_st;
  d_st.tls = c_st.tls;
  e_st.prev = &d_st;
  e_st.tls = d_st.tls;
  f_st.prev = &e_st;
  f_st.tls = e_st.tls;
  g_st.prev = &f_st;
  g_st.tls = f_st.tls;
  validationMatrix_data = validationMatrix->data;
  emlrtHeapReferenceStackEnterFcnR2012b((emlrtConstCTX)sp);
  n1 = 1.0;
  xoffset = numMeas;
  if (numMeas > numTracks) {
    xoffset = numTracks;
  }
  maxval = numMeas;
  if (numMeas < numTracks) {
    maxval = numTracks;
  }
  for (k = 0; k < xoffset; k++) {
    st.site = &dw_emlrtRSI;
    n2 = (real_T)xoffset - (real_T)k;
    b_st.site = &kw_emlrtRSI;
    c_st.site = &lw_emlrtRSI;
    if (n2 < 0.0) {
      emlrtErrorWithMessageIdR2018a(&c_st, &bc_emlrtRTEI,
                                    "MATLAB:factorial:NNegativeInt",
                                    "MATLAB:factorial:NNegativeInt", 0);
    } else if (n2 < 1.0) {
      n = 1.0;
    } else {
      n = dv[(int32_T)n2 - 1];
    }
    st.site = &dw_emlrtRSI;
    n1 += nchoosek(&st, xoffset, k) * nchoosek(&st, maxval, n2) * n;
  }
  emxInit_real_T(sp, &y, 1, &sh_emlrtRTEI, true);
  st.site = &ew_emlrtRSI;
  emxInit_int8_T(&st, &x, &qh_emlrtRTEI);
  xj = x->size[0] * x->size[1];
  x->size[0] = validationMatrix->size[0];
  x->size[1] = validationMatrix->size[1];
  emxEnsureCapacity_int8_T(&st, x, xj, &qh_emlrtRTEI);
  x_data = x->data;
  xoffset = validationMatrix->size[0] * validationMatrix->size[1];
  for (xj = 0; xj < xoffset; xj++) {
    x_data[xj] = (int8_T)validationMatrix_data[xj];
  }
  b_st.site = &vi_emlrtRSI;
  c_st.site = &vh_emlrtRSI;
  maxval = x->size[1];
  d_st.site = &wi_emlrtRSI;
  e_st.site = &mw_emlrtRSI;
  vstride = x->size[0];
  xj = y->size[0];
  y->size[0] = x->size[0];
  emxEnsureCapacity_real_T(&e_st, y, xj, &rh_emlrtRTEI);
  y_data = y->data;
  f_st.site = &qw_emlrtRSI;
  if (x->size[0] > 2147483646) {
    g_st.site = &mb_emlrtRSI;
    check_forloop_overflow_error(&g_st);
  }
  for (xj = 0; xj < vstride; xj++) {
    y_data[xj] = x_data[xj];
  }
  f_st.site = &pw_emlrtRSI;
  if (x->size[1] > 2147483646) {
    g_st.site = &mb_emlrtRSI;
    check_forloop_overflow_error(&g_st);
  }
  for (k = 2; k <= maxval; k++) {
    xoffset = (k - 1) * vstride;
    f_st.site = &ow_emlrtRSI;
    for (xj = 0; xj < vstride; xj++) {
      y_data[xj] += (real_T)x_data[xoffset + xj];
    }
  }
  emxFree_int8_T(&e_st, &x);
  f_st.site = &nw_emlrtRSI;
  st.site = &ew_emlrtRSI;
  b_st.site = &rw_emlrtRSI;
  c_st.site = &vh_emlrtRSI;
  maxval = y->size[0];
  d_st.site = &wh_emlrtRSI;
  n2 = y_data[0];
  e_st.site = &sw_emlrtRSI;
  if (y->size[0] > 2147483646) {
    f_st.site = &mb_emlrtRSI;
    check_forloop_overflow_error(&f_st);
  }
  for (k = 2; k <= maxval; k++) {
    n2 *= y_data[k - 1];
  }
  emxFree_real_T(&d_st, &y);
  n2 = muDoubleScalarRound(muDoubleScalarMin(n1, n2));
  if (n2 < 2.147483648E+9) {
    if (n2 >= -2.147483648E+9) {
      nPotentials = (int32_T)n2;
    } else {
      nPotentials = MIN_int32_T;
    }
  } else if (n2 >= 2.147483648E+9) {
    nPotentials = MAX_int32_T;
  } else {
    nPotentials = 0;
  }
  emlrtHeapReferenceStackLeaveFcnR2012b((emlrtConstCTX)sp);
  return nPotentials;
}

/* End of code generation (numPotentialFeasibleEvents.c) */
