function waypoint_segments = generate_waypoints_door_distances()
%% Interactive Waypoint Generator with Door-to-Door Distances
% Click on the map to select start and goal positions
% Uses actual door-to-door distances for optimal path finding
%
% Usage:
%   segments = generate_waypoints_door_distances()
%
% Instructions:
...
%   3. Waypoint segments will be generated using door-to-door distances

DOOR_OFFSET = 0.5; % meters before/after door

%% Build room network
fprintf('=== Interactive Waypoint Generator (Door-to-Door Distances) ===\n\n');
...
% Connections (cost doesn't matter - door-to-door calculated automatically)
...
fig = figure('Name', 'Interactive Waypoint Generator - Door-to-Door Distances', ...
...
%% Find room sequence using door-to-door distances
fprintf('\n─────────────────────────────────────────────────\n');
fprintf('Finding optimal path using door-to-door distances...\n');
fprintf('From Room %s to Room %s\n', start_room_id, goal_room_id);

[room_sequence, ~, total_door_cost] = graph.findRoomPath(start_room_id, goal_room_id);
...
fprintf('Door-to-door cost: %.2f meters\n', total_door_cost);
fprintf('─────────────────────────────────────────────────\n\n');
...
for i = 1:num_segments
    ...
    if i == 1
        ...
        % Later segments: start from exit of previous door (0.5m after door)
...
fprintf('Total path cost (door-to-door): %.2f meters\n', total_door_cost);
fprintf('=================================================\n\n');
...
function visualize_segments_on_map(segments)
    ...
    title(sprintf('Waypoint Segments (%d segments) - Optimized with Door-to-Door Distances', length(segments)), ...
